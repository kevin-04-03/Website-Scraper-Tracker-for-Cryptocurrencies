...
def collect_data():
    ...
    for attempt in range(max_retries):
        try:
            ...
            if 'result' not in data or 'XETHZUSD' not in data['result']:
                raise ValueError("Invalid data structure in API response")

            ticker_data = data['result']['XETHZUSD']
            ...
            cursor = conn.cursor()
            query = """INSERT INTO ethereum (CreatedDateTime, CurrentPrice, DayHighestPrice, DayLowestPrice) VALUES (%s, %s, %s, %s)"""

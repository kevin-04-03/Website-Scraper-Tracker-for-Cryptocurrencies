...
def collect_data():
    ...
    for attempt in range(max_retries):
        try:
            ...
            if 'result' not in data or 'SOLUSD' not in data['result']:
                raise ValueError("Invalid data structure in API response")

            ticker_data = data['result']['SOLUSD']
            ...
            cursor = conn.cursor()
            query = """INSERT INTO solana (CreatedDateTime, CurrentPrice, DayHighestPrice, DayLowestPrice) VALUES (%s, %s, %s, %s)"""
